...
def main(local_rank, train_set, val_set, test_set, GPU_mode=True):
    ...
    net=ParticleNet.ParticleNet(   input_dims=feature_dim,
    ...
    if local_rank==0:
        ...
        writer.add_graph(net,input_to_model=[torch.rand(1,*points_shape_for_one_event).to(local_rank),
                                             torch.rand(1,*features_shape_for_one_event).to(local_rank)] if GPU_mode else 
                                             [torch.rand(1,*points_shape_for_one_event),
                                             torch.rand(1,*features_shape_for_one_event)])
