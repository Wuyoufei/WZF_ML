...
def pairwise_distance(x):#(batchsize, points_features, num_particles)
    inner = -2 * torch.matmul(x.transpose(2, 1), x)
    xx = torch.sum(x ** 2, dim=1, keepdim=True)
    return (xx.transpose(2, 1) + inner + xx).clamp(min=0)#(batchsize, num_particles, num_particles)


def pairwise_distance_etaphi(x):
    eta, phi = x.split((1, 1), dim=1)#(batchsize, 1, num_particles)
    dphi = delta_phi(phi.unsqueeze(-1), phi.unsqueeze(-2))#(batchsize, 1, num_particles, 1)    (batchsize, 1, 1, num_particles)
    dist = dphi.square().sum(dim=1) + pairwise_distance(eta)
    return dist#(batchsize, num_particles, num_particles)  对称！！
...
def knn(x, k, distance_fn=pairwise_distance_etaphi, exclude_self=False):
    ...
    return idx# # (batch_size, num_particles, k)


def gather(x, k, idx, cpu_mode=False):
    batch_size, num_dims, num_points = x.size()#(batchsize, points_features, num_particles)
    ...
    return fts# (batch_size, num_dims, num_points, k)


def gather_edges(x, idx):# idx  (batch_size, num_particles, k)
    num_dims = x.size(1)
    idx = idx.unsqueeze(1).repeat(1, num_dims, 1, 1)# (b, 1, num_particles, k)-> (b, num_features, num_particles, k)
...
def get_graph_feature(pts=None, fts=None, lvs=None, mask=None, edges=None, idx=None, null_edge_pos=None,
                      k=None,
                      use_rel_fts=False, #relative features
    ...
    if null_edge_pos is None and mask is not None:
        mask_ngbs = gather(mask, k, idx, cpu_mode=cpu_mode)# (batch_size, 1, num_particles, k) -> 每个粒子的k近邻（包含自己by default）粒子是否为mask
        mask_ngbs = mask_ngbs & mask.unsqueeze(-1)          #每个粒子，如果某粒子本身就是pad粒子，那么该粒子的k近邻粒子全为false，即全忽略
        null_edge_pos = ~mask_ngbs   #取反得到：每个粒子的k近邻（包含自己by default）粒子是否为pad粒子，如果某粒子本身就是pad粒子，那么该粒子的k近邻粒子全为false，即全忽略
    ...
    rel_coords = None
    if pts is not None:                     # (batch_size, num_features, num_particles)
        if use_rel_coords or use_rel_dist:
            pts_ngbs = gather(pts, k, idx, cpu_mode=cpu_mode)# (batch_size, num_features, num_particles, k)
...
class SqueezeAndExcitation2d(nn.Module):#为了减少计算量，中间纬度会把channel降到1/4 or 1/16
    ...
    def forward(self, x, mask): #mask is null_edge_pos   (batch_size, 1, num_particles, k)
        with torch.no_grad():
            n = mask.sum((2, 3))
            n.masked_fill_(n == 0, 1)

        b, c, _, _ = x.size()
        y = (x * mask).sum((2, 3)) / n  #pooling
...
class MultiScaleEdgeConv(nn.Module):
    ...
    def __init__(self, node_dim, edge_dim,
        ...
        if reduction_dilation is None:
            reduction_dilation = [(1, 1)]#[(8, 1), (4, 1), (2, 1), (1, 1)]
        for reduction, dilation in reduction_dilation:
            self.slices.append(slice(None, num_neighbors // reduction, dilation))#start stop step
            self.slice_dims.append(num_neighbors // reduction // dilation)#32 16 8 4
    ...
    def forward(self, points, features, lorentz_vectors, mask=None, edges=None,
        ...
        fts_encode = self.node_encode(features).squeeze(-1)# (batch_size, num_features, num_particles, 1) -> (batch_size, message_dim // 2, num_particles)
        ...
        if idx is None:
            ...
            if self.k < self.num_neighbors_in:#maximum 所有MultiScaleEdgeConv块中最大的k
                idx = idx[:, :, :self.k]
                null_edge_pos = null_edge_pos[:, :, :, :self.k]
                if edge_inputs is not None:
                    edge_inputs = edge_inputs[:, :, :, :self.k]
            edges, *_ = self.get_graph_feature(#整除2的原因在这里，会返回2*message_dim//2  （至少）
                fts=fts_encode, mask=mask, edges=edges, idx=idx, null_edge_pos=null_edge_pos)#这里或许有个bug，为什么没有pts###############################
            if edge_inputs is not None:
                edges = torch.cat([edges, edge_inputs], dim=1)

        if sum(self.slice_dims) < self.k:
            edges = torch.cat([edges[:, :, :, s] for s in self.slices], dim=-1)#edges(batchsize, num_features, particles, k)
            null_edge_pos = torch.cat([null_edge_pos[:, :, :, s] for s in self.slices], dim=-1)#s(start, stop, step)

        message = self.edge_mlp(edges)
        if self.edge_se is not None:
            message = self.edge_se(message, ~null_edge_pos)#先是k近邻的粒子全部过一次注意力块

        if sum(self.slice_dims) < self.k:#:(  it's wrong...  maybe?
            message = message.split(self.slice_dims, dim=-1)
            null_edge_pos = null_edge_pos.split(self.slice_dims, dim=-1)
        else:
            message = [message[:, :, :, s] for s in self.slices]#multi scale
            null_edge_pos = [null_edge_pos[:, :, :, s] for s in self.slices]

        pts_out = points
        node_inputs = []
        node_lv_inputs = []
        for i, s in enumerate(self.slices):#multi scale begin
            def masked(x, val=0.):
                return x.masked_fill(null_edge_pos[i], val)

            msg = message[i]#message 存的是多个slices下，经过了node_encode后，点和边的信息

            if self.update_coords:
                coords_weights = masked(self.coords_mlp[i](msg))  # (b, 1, i, k)#这里可能有一个bug，好像不需要mlp的输入好像不需要乘以len(slice_dim)
            ...
            if 'attn' in self.edge_aggregation:
                ...
                if self.msg_repeat is not None:
                    ...
                    msgs = msg
                fts = (attn * msgs).sum(dim=-1, keepdim=True)# (b, dim, particles) 之后各个slice的k近邻分别过一次注意力块
            else:
                msg = masked(msg)
        ...
        node_fts = self.node_mlp(torch.cat(node_inputs, dim=1))#message_dim * len(self.slices) 在这里是正常的
        if self.node_se is not None:
            node_fts = self.node_se(node_fts, mask.unsqueeze(-1))#多尺度k近邻信息的多头注意力

        fts_out = self.shortcut(features) + self.gamma * node_fts# 4D+4D  please make sure keepdim=True

        return pts_out, fts_out#更新（or没更新）之后的坐标  和  多尺度k近邻信息的融合
...
class ParticleNeXt(nn.Module):
    ...
    def __init__(self,
                 ...
                 edge_dim=8,
                 use_node_bn=True,
                 use_edge_bn=True,    #k, out_dim, rd, msg_dim
        ...
        # input augmentation
        self.input_dropout = nn.Dropout(input_dropout) if input_dropout else None #None
        self.pt_dropout = pt_dropout
        self.lorentz_vector_scale = lorentz_vector_scale
        self.lorentz_vector_smear = lorentz_vector_smear
        self.lorentz_vector_shift = lorentz_vector_shift                 #ellipsis...
        ...
        if node_dim:
            ...
            )                                                         #to map the [feature_input_dim] to [node_dim]
            input_dim = node_dim   #now the input_dim is node_dim
        else:
        ...
        for param in layer_params:
            ...
            self.layers.append(
                MultiScaleEdgeConv(
                    ...
                    message_dim=msg_dim,
                    # more options
                    edge_aggregation=edge_aggregation,              #attn8
                    use_rel_lv_fts=use_rel_lv_fts,                  #True
                    use_polarization_angle=use_polarization_angle,  #False
                    use_rel_fts=use_rel_fts,                        #False
                    use_rel_dist=use_rel_dist,                      #False
                    update_coords=update_coords,                    #False
                    lv_aggregation=lv_aggregation,                  #False
                    use_node_se=use_node_se,                        #True
                    use_edge_se=use_edge_se,                        #True
                    init_scale=init_scale,                          #1e-5
                    cpu_mode=for_inference,                         #False
                )
            )
            num_neighbors.append(k)
            input_dim = out_dim  #concat the MultiScaleEdgeConv block

        self.num_neighbors = max(num_neighbors) #maximum
        if not update_coords:#False
            ...
            for i in range(len(self.layers)):
                self.layers[i].num_neighbors_in = self.num_neighbors#maximum 所有MultiScaleEdgeConv块中最大的k
        ...
        self.global_aggregation = global_aggregation.lower()#'attn4'
        ...
        self.for_segmentation = for_segmentation #False
    ...
    def forward(self, points, features, lorentz_vectors, mask=None, edges=None):
        ...
        with torch.no_grad():
            ...
            if self.training:
                ...
                if self.lorentz_vector_scale:#None
                ...
                if self.lorentz_vector_smear:#None
                ...
                if self.lorentz_vector_shift:#None
                ...
                if self.lorentz_vector_scale is not None or self.lorentz_vector_smear is not None or self.lorentz_vector_shift is not None:#None
        ...
        if self.for_segmentation:
            ...
            if 'attn' in self.global_aggregation:
                attn = masked(self.sa(features), -np.inf)#(batchsize, num_features, num_particles)
                attn = attn.softmax(dim=-1)
                if self.sa_repeat is not None:
                    attn = attn.repeat(1, self.sa_repeat, 1)#多头注意力机制  
